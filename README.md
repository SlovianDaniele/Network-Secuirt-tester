...
Для роботи додатку потрібна віртуальна машина Kali Linux з попередньо налаштованими інструментами пентестування.

### Завантаження образу

Готовий образ віртуальної машини доступний для завантаження:

📥 [Завантажити образ Kali Linux VM](https://drive.google.com/file/d/1TZMxhe2tSGJeQhAbe4FCpYbNxegda0UU/view?usp=sharing)

### Встановлення та налаштування

1. **Завантажте образ** з посилання вище
2. **Імпортуйте VM** в Oracle VirtualBox:
    - Відкрийте VirtualBox
    - Виберіть `File` → `Import Appliance`
    - Виберіть завантажений файл образу
    - Налаштуйте параметри VM (CPU, RAM) відповідно до ваших можливостей
3. **Перевірте назву VM**: За замовчуванням додаток очікує VM з назвою "MAN". Якщо ваша VM має іншу назву, оновіть параметр `vm_name` в `lib/config.py`
4. **Налаштуйте SSH**:
    - За замовчуванням використовуються наступні параметри:
        - Username: `kali`
        - Password: `kali`
        - Port: `22`
    - Якщо ваші параметри відрізняються, оновіть їх в `lib/config.py`
5. **Перевірте підключення**: Запустіть додаток та перевірте статус VM в інтерфейсі

### Вимоги до VM

-   **Мінімальні параметри**: 2 CPU, 4 GB RAM
-   **Рекомендовані параметри**: 4 CPU, 8 GB RAM
-   **SSH доступ**: Повинен бути увімкнений та доступний з хостової системи
-   **Інструменти**: В образі вже встановлені всі необхідні інструменти (wash, airodump-ng, reaver, hashcat)
